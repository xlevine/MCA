...
# Outputs of PCA: E1_N[x,y]; P_1[t] (for each leading modes)
...
varlist_Amon = ['clt','evspsbl','hurs','pr','rlds','rsds','rsut','ta','tasmin','ttrea','utendepfd','va','vurea','wurea','epfy','hfls','hursmin','prc','rlus','rsdt','sfcWind','tas','tos','ua','utrea','vas','vvrea','wvrea','epfz','hfss','huss','psl','rlut','rsus','siconca','tasmax','ts','uas','uurea','vtrea','wap','zg']
...
input_dict={'varname1':'siconc','season1':'winter','region1':'TotalArc','year_start':2015,'year_end':2100}
...
def main():
    ...
    # P3: Plotting
    plot_pca(varnorm,var1_modes_patterns,var1_modes_timeseries,lon1,lat1)
...
def define_pca_inputs():
    ...
    [var1,lon1,lat1,area1] = load_input_var(varname1,season1,member_start,member_end) 
...
def compute_pca(dataset1,area1,region1,lon1,lat1):
    ...
    [u, v, var1_total_mask, varnorm] = pca_1D(var1_yearseries_domain_1D,var1_total_mask)
...
## P3: Plotting
def plot_pca(varnorm,var1_modes_patterns,var1_modes_timeseries,lon1_2d,lat1_2d):
...
def load_input_var(varname1, season1, member_start, member_end):
    ...
    for memb in nmemb:
        print(memb)
        [var1_memb, lon1, lat1] = load_var(memb,varname1,season1)
        var1.append(var1_memb)
    var1 = np.asarray(var1)
        
    return var1, lon1, lat1, area1

def load_var(num_member, varname, month_name):    
    if month_name in month_list:
        [var, lon_2d, lat_2d] = read_var_month(num_member, varname, month_name)
    else:
        var_season = []
        for month in season_dict[month_name]: 
            [var_month, lon_2d, lat_2d] = read_var_month(num_member, varname, month)
...
### P2: Performing MCA
def pca_1D(var1_yearseries_domain_1D,var1_total_mask):
    ...
    return u, v, var1_total_mask, varnorm 
