...
varlist_Omon = ['msftyz', 'mlotst', 'evs','friver','hfds','mlotst','sos','vmo','ficeberg2d','rsntds','nshfls','downempfluxoce','downnoswfluxoce','downswfluxoce', 'thetao', 'so', 'tos', 'uo', 'vo']
...
def load_input_var_2D(varname1, season1, member_start, member_end):
    nmemb=np.arange(member_start,member_end+1,1)
    var1_2D = []
    area1 = compute_cell_area(varname1)
    for memb in nmemb:
    ...
    return var1_2D, plev1, lat1, area1
...
def compute_zsum(var1):
    var1_zsum =  np.nansum(var1,axis=-1)

    return var1_zsum

def load_var(num_member, varname, month_name):    
    if month_name in month_list:
        [var, lon_2d, lat_2d, plev_2d] = read_var_month(num_member, varname, month_name)
    else:
        var_season = []
